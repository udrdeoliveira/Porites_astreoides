  ...
  ### extract values from occurrences points and study area
  #### Download environmental layers
    dir.create("var")
    dir.create("var/present")
    dir.create("var/paleo")
    ##### present
    load_env_pres <- load_layers(layercodes = c("MS_biogeo08_sss_mean_5m",
                                           ...
                                           "MS_bathy_5m"),
                            equalarea = FALSE, rasterstack = FALSE,
                            datadir = "var/present")
    ##### paleo
    load_env_pal <- load_layers(layercodes = c("MS_biogeo08_sss_mean_6kya",
                                                "MS_biogeo09_sss_min_6kya",
                                                "MS_biogeo10_sss_max_6kya",
                                                "MS_biogeo11_sss_range_6kya",
                                                "MS_biogeo13_sst_mean_6kya",
                                                "MS_biogeo14_sst_min_6kya",
                                                "MS_biogeo15_sst_max_6kya",
  ...
  # load environmental layers
    ...
    #
    pat_var_pres <- "var/present"
    list_var_pres <- list.files(pat_var_pres)
    var_pres_name <- c(
      "bathy.tif",
      "sss_mean.tif",
      "sss_min.tif",
    ...
    # Verifique se o número de novos nomes é igual ao número de arquivos
    if (length(list_var_pres) == length(var_pres_name)) {
      # Renomear os arquivos
      for (i in 1:length(list_var_pres)) {
        file.rename(paste(pat_var_pres, list_var_pres[i], sep="/"),
                    paste(pat_var_pres, var_pres_name[i], sep="/"))
      }
    }
    ...
    # holocene layers
    #
    pat_var_pal <- "var/paleo"
    list_var_pal <- list.files(pat_var_pal)
    var_pal_name <- c(
      "bathy.tif",
      "sss_mean.tif",
      "sss_min.tif",
    ...
    # Verifique se o número de novos nomes é igual ao número de arquivos
    if (length(list_var_pal) == length(var_pal_name)) {
      # Renomear os arquivos
      for (i in 1:length(list_var_pal)) {
        file.rename(paste(pat_var_pal, list_var_pal[i], sep="/"),
                    paste(pat_var_pal, var_pal_name[i], sep="/"))
      }
    }
    ##### load new names
    env_pal_pat <- stack(
    ...
    cbind(occ_cur[, 2:3], raster::extract(env_pre_pat, occ_cur[, 2:3]))
  ...
  za1_plot <- ggplot(za1_lyr) +
    ...
    labs(x = "PC1", y = "PC2") +
    theme_minimal() +
    annotate("text", x = 15, y = -18, label = "(c)", size = 6,
             color = "white") +
    theme(plot.title = element_text(size = 10)) +
  ...
  za2_plot <- ggplot(za2_lyr) +
    ...
    labs(x = "PC1", y = "PC2") +
    annotate("text", x = 15, y = -18, label = "(d)", size = 6,
             color = "white") +
    theme_minimal()  +
    theme(plot.title = element_text(size = 10)) +
    theme(legend.position = "none") +
  ...
  dir.create("var/ENMTML")
  dir.create("var/ENMTML/present")
  dir.create("var/ENMTML/present/pca")

  dir.create("var/ENMTML/paleo")
  dir.create("var/ENMTML/paleo/pca")
  ##### Present layers
  ##### Criate a PCA layers
